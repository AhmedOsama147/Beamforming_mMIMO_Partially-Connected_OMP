...
cap_OMP = zeros(lenSNR,1);
Capacity_NKP = zeros(num,1);
cap_NKP = zeros(lenSNR,1);
Capacity_SIC = zeros(num,1);
cap_SIC = zeros(lenSNR,1);
Capacity_mod = zeros(num,1);
cap_mod = zeros(lenSNR,1);
...
for k = 1:lenSNR
    ...
    for i = 1:num
        ...
        for j=1:Ncl
         ...
         Capacity_OMP(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*FRF*FBB*FBB'*FRF'*H_n'));
        
         [FRF_NKP,FBB_NKP] = NKP(NS,NT_RF,NT/NT_RF,Nray,Fopt);
         Capacity_NKP(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*FRF_NKP*FBB_NKP*FBB_NKP'*FRF_NKP'*H_n'));
         
         [FRF_SIC,FBB_SIC] = SIC(NS,NT_RF,NT/NT_RF,NR,H_n,SNR(k));
         Capacity_SIC(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*FRF_SIC*FBB_SIC*FBB_SIC'*FRF_SIC'*H_n'));
         
         [FRF_mod,FBB_mod] = Mod_OMP(NS,NT_RF,NT/NT_RF,Fopt,At);
         Capacity_mod(i) = log2(det(eye(NR)+(EN(k)/NS)*H_n*FRF_mod*FBB_mod*FBB_mod'*FRF_mod'*H_n'));
    end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot(SNR,cap_full_digital,'-ks','LineWidth',3,'MarkerSize',3,'MarkerEdgeColor','black','MarkerFaceColor',[.0 .0 .0]);
hold on;
plot(SNR,cap_OMP,'-ys','LineWidth',0.75,'MarkerSize',3,'MarkerEdgeColor','yellow','MarkerFaceColor',[1 .8 .0]);
hold on;
plot(SNR,cap_NKP,'-rs','LineWidth',0.75,'MarkerSize',3,'MarkerEdgeColor','red','MarkerFaceColor',[1 .6 .6])
hold on;
plot(SNR,cap_SIC,'-bs','LineWidth',0.75,'MarkerSize',3,'MarkerEdgeColor','blue','MarkerFaceColor',[.3 .5 .8])
hold on;
plot(SNR,cap_mod,'-gs','LineWidth',0.75,'MarkerSize',3,'MarkerEdgeColor','green','MarkerFaceColor',[0 .5 .5]);
hold on;
xlim([minSNR maxSNR]);
ylim([minCap maxCap]);
grid on;
